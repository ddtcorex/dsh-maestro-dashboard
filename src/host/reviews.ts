import type { ReviewsSnapshot } from './shared/types.ts'
...
export async function getReviewsSnapshot(limit = 20): Promise<ReviewsSnapshot> {
  ...
  try {
    ...
    for (const p of settingsPaths) {
      ...
      if (typeof b === 'string' && b) { gitlabBaseUrl = b.replace(/\/$/, ''); break }
    }
  } catch {}
  if (!gitlabBaseUrl) gitlabBaseUrl = 'https://git.sutunam.com'
  ...
  try {
    if (existsSync(altFile)) raw = readFileSync(altFile, 'utf8')
    else if (existsSync(file)) raw = readFileSync(file, 'utf8')
    else if (existsSync(altFile2)) raw = readFileSync(altFile2, 'utf8')
  } catch {}
  if (!raw) {
    return {
      v: 1,
  ...
  try {
    ...
    const sorted = [...arr].sort((a: any, b: any) => (b.startedAt ?? 0) - (a.startedAt ?? 0)).slice(0, Math.min(100, Math.max(1, limit)))
    const reviews = sorted.map((r: any) => ({
      id: String(r.id ?? ''),
