...
function tryReadVersionFile(pkgJsonPath) {
    try {
...
function getMaestroPackagesDir() {
    try {
        ...
        if (existsSync(altPkg)) {
            try {
                ...
                if (entries.some((e) => e.startsWith('dsh-maestro-')))
                    return altPkg;
            }
            catch { }
        }
...
function collectPatchIds(parsed) {
    const out = [];
    const seen = new Set();
    const push = (raw) => {
        const base = normalizeKey(raw);
        if (!base.startsWith('dsh-maestro-'))
            return;
        if (seen.has(base))
            return;
        seen.add(base);
        ...
        if (Array.isArray(parsed)) {
            for (const el of parsed) {
                if (!el || typeof el !== 'object')
                    continue;
                if (typeof el.id === 'string' && isMaestroKey(el.id))
                    push(el.id);
                if (Array.isArray(el.bundles)) {
                    for (const b of el.bundles)
                        if (typeof b === 'string' && isMaestroKey(b))
                            push(b);
                }
                if (Array.isArray(el.insert)) {
                    for (const b of el.insert) {
                        if (b && typeof b === 'object' && typeof b.id === 'string' && isMaestroKey(b.id))
                            push(b.id);
                    }
                }
            }
            ...
            if (dsh && dsh.profile && Array.isArray(dsh.profile.bundles)) {
                for (const b of dsh.profile.bundles)
                    if (typeof b === 'string' && isMaestroKey(b))
                        push(b);
            }
        }
...
export async function getPluginsSnapshot(opts = {}) {
    const generatedAt = Date.now();
    try {
        // Tolerant yaml parse for patchYml
        let patchContent = opts.patchYml ?? null;
        if (patchContent === null) {
            try {
                const p = join(homedir(), '.dsh', 'profiles', 'web', 'cordis.patch.yml');
                if (existsSync(p))
                    patchContent = readFileSync(p, 'utf8');
            }
            catch { }
        }
        let parsedPatch = null;
        let patchHealth = null;
        if (patchContent) {
            try {
                ...
                try {
                    parsedPatch = yaml.load(patchContent, { schema: yaml.FAILSAFE_SCHEMA });
                }
                catch { }
        ...
        const health = [{ id: 'plugins', status: 'ok' }];
        if (patchHealth)
            health.unshift(patchHealth);
        return {
            v: 1,
            generatedAt,
